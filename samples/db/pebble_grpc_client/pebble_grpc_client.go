...
type Line struct {
	...
	PebbleClient kvv1.KvServiceClient
}
...
var LineSet = wire.NewSet(
	...
	pebbleservice.NewPebbleGrpcClient,
...
func main() {
	...
	_, err = line.PebbleClient.Set(context.Background(), &kvv1.SetRequest{
	...
	res, err := line.PebbleClient.Get(context.Background(), &kvv1.GetRequest{Key: key})
	if err != nil {
		dot.Logger.Error().Err(err).Send()
		return
	}
	if !bytes.Equal(value, res.Value) {
		dot.Logger.Error().Msg("the get value dont eq value")
		return
	} else {
